        ...
        **/
        ...
        for (auto& kv : m_clients)
        {
            ...
            e_cal_client_generate_instances(client,
                                            ...
                                            new AppointmentSubtask(main_task, client, color, default_timezone, begin_timet, end_timet),
                                ...
                                gpointer       gsubtask)
    {
        ...
        if ((vtype == E_CAL_COMPONENT_EVENT) || (vtype == E_CAL_COMPONENT_TODO))
        {
                ...
                (status != ICAL_STATUS_CANCELLED))
            {
                ...
                std::cerr << "begin[" << DateTime(begin).format("%F %T") << "] "
                          << "end[" << DateTime(end).format("%F %T") << "] "
                          << "e_alarms[" << e_alarms << "] "
                          << std::endl;
                ...
                if (e_alarms != nullptr)
                {
                    std::cerr << "found " << g_slist_length(e_alarms->alarms) << " alarm instances" << std::endl;
